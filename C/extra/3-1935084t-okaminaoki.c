...
double solve(int nTickets, int mCities, int pPaths, int a, int b) {
    ...
    while(qSize(&Q) > 0) {
        ...
        // TODO
        // 各自で完成させてみましょう。
        if(here.city==b)return keiro[b]/2;                          //
        for(int i=0;i<mCities;i++){                                 //
            if(100>=road[here.city][i]&&road[here.city][i]>=0){     //
                if(keiro[i]>road[here.city][i]+keiro[here.city]){   //
                    keiro[i]=road[here.city][i]+keiro[here.city];   //
                    searchNode_t siten={ keiro[i]/2,i,0 };          //
                    enqueue(&Q,siten);                              //
                }                                                   //
            }                                                       //
        }                                                           //
...
int main(void) {
    ...
    struct {
        ...
        { {"sample.in", "sampleE.ans", "sample.ans"},
          {"D1.in", "D1E.ans", "D1.ans"}}; 

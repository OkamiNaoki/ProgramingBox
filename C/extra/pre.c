...
#define ITERATION 10000 /* Number of Iteration 反復回数 */ 
#define ETA 0.007 /* Learning coefficeint 学習係数 */
...
int load_data(float x[][INPUT_DIM], float y[][OUTPUT_DIM]) {
    ...
    if ((ifp = fopen("kadai_data_mlp_2021.dat", "r")) == NULL) { /* データファイル名 */
...
int main(void) {
    ...
    for (t = 0; t < ITERATION; t++) {
        ...
        if (t == ITERATION - 1) {
            ...
            if ((ofp2 = fopen("prediction_m.dat", "w")) == NULL) {
                printf("File cannot be opened.\n");
                exit(0);
            }
    ...
    float input_x;
    FILE *ofp3;
    float print_y[MAX_N_DATA][OUTPUT_DIM];
    ...
    for (n = 0; n < 600; n++) {
        ...
        for (j = 0; j < OUTPUT_DIM; j++) {
            ...
            a1[n][j] += b1[j];
            print_y[n][j]=act_func_2nd(a1[n][j]); /* 隠れ層→出力層の活性化関数 */
        }
    ...
    //ファイル出力
    if ((ofp3 = fopen("prediction2_m.dat", "w")) == NULL) {
        printf("File cannot be opened.\n");
        exit(0);
    }
    ...
    for (n = 0; n < 600; n++) {
        for (i = 0; i < INPUT_DIM; i++) {
            fprintf(ofp2, "%f\t", input_x);
        }

        for (j = 0; j < OUTPUT_DIM; j++) {
            fprintf(ofp3, "%f\t", print_y[n][j]);
        }

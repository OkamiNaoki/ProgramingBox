...
int main(void) {
    ...
    FILE *ofp3;
    float y_pre[MAX_N_DATA][OUTPUT_DIM];
    for (n = 0; n < 600; n++) {
        for (j = 0; j < HIDDEN_DIM; j++) {
            a0[n][j] = 0;
            for (i = 0; i < INPUT_DIM; i++) {
                a0[n][j] += w0[j][i] * (-2+0.02*n);
            }
            a0[n][j] += b0[j];
            h[n][j] = act_func_1st(a0[n][j]); 
        }
        ...
        for (j = 0; j < OUTPUT_DIM; j++) {
            ...
            a1[n][j] += b1[j];
            y_pre[n][j]=act_func_2nd(a1[n][j]); /* 隠れ層→出力層の活性化関数 */
        }
    }

    //ファイル出力
    if ((ofp3 = fopen("prediction_1.dat", "w")) == NULL) {
        printf("File cannot be opened.\n");
        exit(0);
    }
    ...
    for (n = 0; n < 600; n++) {
        for (i = 0; i < INPUT_DIM; i++) {
            fprintf(ofp2, "%f\t", -2+0.02*n);
        }

        for (j = 0; j < OUTPUT_DIM; j++) {
            fprintf(ofp3, "%f\t", y_pre[n][j]);
        }

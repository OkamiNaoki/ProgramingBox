...
int solve(int w, int h) {
    point_t start = {0, 0};
    int keiro[30][30];                                      //
    for(int i=0;i<w;i++)for(int j=0;j<h;j++)keiro[i][j]=0;  //
    keiro[0][0]=1;                                          //
    ...
    while(qSize(&strQ) > 0) {
        ...
        /* TODO */
        for(int i=0;i<4;i++){                                                                               //
            if(canGo(here,directions[i])==1&&(keiro[here.x+directions[i].x][here.y+directions[i].y]==0)){   //
                keiro[here.x+directions[i].x][here.y+directions[i].y]=keiro[here.x][here.y]+1;              //
                if((here.x+directions[i].x==w-1)&&(here.y+directions[i].y==h-1)){                           //
                    return keiro[here.x+directions[i].x][here.y+directions[i].y];                           //
                }                                                                                           //
                point_t start = {(here.x+directions[i].x),here.y+directions[i].y};                          //
                    enqueue(&strQ,start);                                                                   //
            }                                                                                               //
        }                                                                                                   //
